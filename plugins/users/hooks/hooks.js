...
var hooks = {
  ...
  afterListPageDraft: function(workflow, file, index, text, abe) {
    ...
    if(typeof file.draft !== 'undefined' && file.draft !== null
      ...
      Array.prototype.forEach.call(workflowUser, (flow) => {
        ...
        if(status === flow) {
          newRes += `<a href="/abe/${file.template}?filePath=${file.draft.cleanFilePath}" class="label label-default label-draft">
            ${flow}
            </a>`
        }else {
          newRes += ``
      ...
      newRes += `<td align="center publish">`
      if (file.published){
      ...
      Array.prototype.forEach.call(workflowUser, function(flow) {
        newRes += `<td align="center"></td>`
      })
      newRes += `<td align="center">
        <a href="/abe/${file.template}?filePath=${file.path}" class="checkmark">&#10004;</a>
      </td>`
    }
  ...
  afterExpress: function(app, express, abe) {
    ...
    app.use( function( req, res, next ) {
      var isNexted = false
      if( req.url === '/plugin/users/login' && req.method === 'POST' ) {
